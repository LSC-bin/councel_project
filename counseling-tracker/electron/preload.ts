...
const api = {
  ...
  getStudentRanking: (limit = 10, periodDays?: number) => ipcRenderer.invoke('stats:studentRanking', limit, periodDays),
  getClassHeatmap: (periodDays?: number) => ipcRenderer.invoke('stats:classHeatmap', periodDays),
  getTypeTrend: (months = 6) => ipcRenderer.invoke('stats:typeTrend', months),
  getClassSummary: (periodDays?: number) => ipcRenderer.invoke('stats:classSummary', periodDays),
  getPendingActionsSummary: () => ipcRenderer.invoke('actions:pendingSummary'),
  exportDataJson: () => ipcRenderer.invoke('data:exportJson'),
  importDataJson: (mode: 'merge' | 'replace') => ipcRenderer.invoke('data:importJson', mode),
